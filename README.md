...
├── adapter/m231/              Пакет-адаптация решения заказчика (Thymeleaf+jQuery+Chart.js)
└── .github/workflows/         CI и публикация на GitHub Pages
```

## Адаптация под решение заказчика (`adapter/m231/`)

Параллельно с собственной React-версией в репозитории лежит пакет
`adapter/m231/` — drop-in правки для уже существующего у заказчика решения
(Spring Boot + Thymeleaf + jQuery + Chart.js + DataTables + Tom Select).
В нём:

- `ReportController.java` с добавленной колонкой `Дата решения` и
  параметризованными запросами (закрыта SQL-инъекция исходного кода);
- два независимых графика вместо одного (stacked «Поступившие / Решённые»
  и гистограмма открытых ЕИ по 6 бакетам с мини-KPI);
- фильтр «Срок решения»;
- вынесенные чистые функции в `m231-core.js` и unit-тесты на `node:test`;
- локальный `preview.html`, подменяющий API на моковую фикстуру.

Подробности и чек-лист проверки — в [`adapter/m231/README.md`](adapter/m231/README.md).
...
│   └── aggregate.ts         KPI, активность, гистограмма открытых, пивоты
├── state/filters.ts         Zustand-стор фильтров
├── components/
│   ├── layout/              Сетка дашборда
│   ├── filters/             Сайдбар
│   ├── kpi/                 KPI-карточка
│   ├── charts/              ActivityChart (динамика) + ResolutionHistogram (снимок)
...
3. Фронт получает плоский список и в каждом рендере применяет фильтры + считает
   KPI, динамику (поступившие/решённые с разбиением по МИ + средняя длительность),
   снимок открытых ЕИ по 6 часовым бакетам и пивоты в чистых функциях
   `lib/aggregate.ts`.
...
Тесты (vitest, 22 штук) покрывают `lib/`:
...
- `aggregate` — все ключевые функции: `applyFilters`, `computeKpi`,
  `computeMonthly` (хронологический порядок), `resolvedDateOf` (дата решения
  = `createdAt + resolutionHours`), `computeActivity` (стеки с МИ/без МИ для
  поступивших/решённых, средняя длительность по решённым),
  `hourBucket` и `computeResolutionHistogram` (6 часовых бакетов `4ч..>72ч`,
  учёт только открытых), оба пивота (срок решения с соблюдением порядка,
